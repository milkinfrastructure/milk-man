...
- The deterministic Python engine from checkpoint
  `12fd42d7f039b55ae61a479543b5f4caa182784c` is now owned by the checked-in
  `milk-jobs` package with its production and mechanics configurations. It is
  not a third product, service, scheduler, or authority.
...
`milk_jobs.reconcile()` loads the operator-pinned in-repository configuration
and performs one finite pass: summary, classification, readiness, eval
generation, validation, candidate scoring, and unsigned proposal. There is no
external runtime checkout in the job path.
...
`milk/self-improve.sh` accepts one reviewed task string. Its fixed launcher,
SRT policy, autonomous limits, and repository gate are the enforced contract;
there is no second task schema that only documents permissions. A run may also
receive a reviewed, bounded Codex message range through
`milk/codex-context.sh` as advisory history. The adapter retains only user and
assistant text and rejects unbounded or oversized input. Do not migrate a full
transcript, hidden reasoning, tool calls or results, raw credentials, browser
sessions, shell history, memory folders, production prompts or responses,
signing keys, implicit approvals, or an unreviewed dirty checkout.

## Minimum local loop

1. Start from an exact clean Milk Man commit and a dedicated state directory.
2. Run `milk/self-improve.sh` with one reviewed task.
3. Let the sandboxed agent edit only its retained disposable worktree and run
   the fixed repository gate.
4. Stop on success or the fixed turn, time, or token limits.
5. Review the retained branch before copying or committing its diff.
6. Run `milk/jobs.sh` separately when the operator intends object-store or
   provider work; its checked-in config and envvars bound that call.
7. Keep push, deploy, route signing, and route publication as operator actions.
